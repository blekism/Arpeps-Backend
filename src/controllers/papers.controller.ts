...
export const getMarkdownControllerSingle = async (
  req: Request,
  res: Response,
) => {
  const user_id = (req as any).user_id; // id getter
  ...
  try {
    // pass id to services
    const concepts = await getMarkdownPaperSingle(user_id, paper_id);
...
export const getMapControllerSingle = async (req: Request, res: Response) => {
  const user_id = (req as any).user_id; // id getter
  ...
  try {
    // pass id to services
    const concepts = await getPaperMapService(user_id, paper_id);
